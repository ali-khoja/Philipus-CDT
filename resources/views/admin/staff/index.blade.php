...
@section('main')
        ...
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$c->name}}</td>
                        <td><img src="{{ asset('images/staff/' . $c->picture) }}" width="100px" height="100px" alt="{{ $c->name }}">
                        </td>
                        <td>{{$c->position}}</td>
                        <td>{{$c->description}}</td>
                        <td>
	...
	<div id="editpicture{{$c->id}}" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
				<form action="{{route('staff.update' , $c->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <img src="{{ asset('images/staff/' . $c->picture) }}" width="75px" height="75px" alt="{{ $c->name }}">
                    </div>
					<div class="modal-body">
						<div class="form-group">
							<label>position</label>
							<input type="text" name="position" value="{{$c->position}}" class="form-control" required>
						</div>
						<div class="form-group">
							<label>Description</label>
                            <textarea name="description" class="form-control" rows="4" cols="50" >{{$c->description}}</textarea>
						</div>
	...
	<div id="deletegallery{{$c->id}}" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
				<form action="{{route('staff.destroy', $c->id)}}" method="POST">
                    @csrf
                    @method('delete')
					<div class="modal-header">
